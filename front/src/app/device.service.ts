...
const RE1x = new RegExp([
  /(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine/,
  /|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo/,
  /|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket/,
  /|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/
].map(r => r.source).join(''), 'i');

const RE2x = new RegExp([
...
export class DeviceService {
  ...
  get isMobile(): boolean {
    return (RE1x.test(navigator.userAgent) || RE2x.test(navigator.userAgent.substr(0,4)));
  }
